...
namespace Persistence
{
    static class SpecificationEvalutor
    {
        public static IQueryable<TEntity> GetQuery<TEntity,TKey>(IQueryable<TEntity> inputQuery,ISpecifications<TEntity,TKey> spec) where TEntity : BaseEntity<TKey>
        {
            ...
            //foreach(var item in spec.IncludeExprerssions)
            //{
            //   query= query.Include(item);
            //}
            query = spec.IncludeExprerssions.Aggregate(query, (currentQuery, includeExpression) => currentQuery.Include(includeExpression));
            return query;
